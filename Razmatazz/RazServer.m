...
@property (nonatomic, strong) NSMutableSet *         connectionsMutable;
@property (nonatomic, strong) NSMutableSet *         runLoopModesMutable;
@property (nonatomic, strong) NSMutableSet *         listeningSockets;
@property (nonatomic, assign) NSUInteger             registeredPort;
@property (nonatomic, assign) NSUInteger             connectionSequenceNumber;

@end

@implementation RazServer

- (id) initWithPreferredPort:(NSUInteger)preferredPort andPartyName:(NSString*)partyName{
    self = [super init];
    
    if(self){
        self.connectionsMutable = [[NSMutableSet alloc] init];
        self.runLoopModesMutable = [[NSMutableSet alloc] initWithObjects:NSDefaultRunLoopMode, nil];
        self.listeningSockets = [[NSMutableSet alloc] init];
...
- (id) initWithPreferredPort:(NSUInteger)preferredPort {
    self = [self initWithPreferredPort:preferredPort andPartyName:nil];
...
- (NSSet*) connections {
    return [self.connectionsMutable copy];
}
...
// See comment in header.
{
    ...
    do {
        ...
        if (err == 0) {
            ...
            // port is in use, try binding to 0, which causes the kernel to choose a port for us.
...
// method.
{
    ...
    obj = (__bridge RazServer *) info;
    assert([obj isKindOfClass:[RazServer class]]);
